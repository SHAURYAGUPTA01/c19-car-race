...
function setup() {
  ...
  car.setCollider("rectangle",0,0,250,410);
  car.debug = false;

  if(!localStorage["hiDistance"]){
    localStorage["hiDistance"] = 0;
  }
}
...
if(gameState === END){
  ...
  if(path.y > 400){
    ...
    text("HIGH DISTANCE : " + localStorage["hiDistance"],80,80)
}

function restart(){
  gameState = PLAY;
  if(distance > localStorage["hiDistance"]){
    localStorage["hiDistance"] = distance;
  }
  distance = 0;
}
...
function spawncar1(){
  if(frameCount % 300 === 0){
   ...
   oppCar1G.depth = obstacle1G.depth;
   oppCar1G.depth = obstacle2G.depth;
   oppCar1G.depth +=1;
  }
}
...
function spawncar2(){
  if(frameCount % 250 === 0){
   ...
   oppCar2G.depth = obstacle1G.depth;
   oppCar2G.depth = obstacle2G.depth;
   oppCar2G.depth +=1;
  }
}
